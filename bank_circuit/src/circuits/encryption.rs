...
use crate::gadgets::public_encryptions::{constraints::AsymmetricEncryptionGadget, elgamal};
    ...
    C::BaseField: PrimeField,
{
    fn generate_constraints(
        ...
        // instance
        let pk = elgamal::constraints::PublicKeyVar::new_input(cs.clone(), || {
            self.instance.pk.ok_or(SynthesisError::AssignmentMissing)
        })?;
        let ct = elgamal::constraints::OutputVar::new_input(cs.clone(), || {
            self.instance.ct.ok_or(SynthesisError::AssignmentMissing)
        })?;

        // witness
        let m = elgamal::constraints::PlaintextVar::new_witness(cs.clone(), || {
            self.witness.m.ok_or(SynthesisError::AssignmentMissing)
        })?;
        let r = elgamal::constraints::RandomnessVar::new_witness(cs.clone(), || {
